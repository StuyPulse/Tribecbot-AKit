...
public class Superstructure extends FullSubsystem {
    ...
    public boolean shouldStop() {
        ...
        boolean isBehindHubWhileFerrying =
                state == SuperstructureState.FOTM && swerve.isBehindHub();
        ...
        boolean isBtwnOppHubAndWall =
                swerve.isBtwnOppHubAndWall() && state == SuperstructureState.FOTM;
    ...
    public void periodicAfterScheduler() {
        ...
        if (Drive.getInstance().isOutsideAllianceZone()
                ...
                && !DriverStation
                        .isAutonomous()) { // allows us to start SOTM earlier in auto, but currently
            // not desired
            // in teleop
    ...
    public Command leftCorner() {
        return setStateCommand(SuperstructureState.LEFT_CORNER)
                .withName("Superstructure Left Corner");
    }
                ...
                .until(
                        () -> {
                            double driverInputMagnitude =
                                    new Translation2d(-driver.getLeftY(), -driver.getLeftX())
                                            .getNorm();
