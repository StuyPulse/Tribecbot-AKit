...
public class LED extends FullSubsystem {
    ...
    private void applyState() {
        ...
        outputs.patterns.add(new LEDPattern(LEDSettings.STRIP_START, LEDSettings.LED_LENGTH, state.getColor()));

        // add limelight dead strips
        if (vision.isCameraDead(Camera.LEFT)) {
            outputs.patterns.add(new LEDPattern(LEDSettings.LEFT_DEAD_START, LEDSettings.LEFT_DEAD_END, LEDSettings.StateColors.LLDEAD));
        }
        if (vision.isCameraDead(Camera.RIGHT)) {
            outputs.patterns.add(new LEDPattern(LEDSettings.RIGHT_DEAD_START, LEDSettings.RIGHT_DEAD_END, LEDSettings.StateColors.LLDEAD));
        }
        if (vision.isCameraDead(Camera.BACK)) {
            outputs.patterns.add(new LEDPattern(LEDSettings.BACK_DEAD_START, LEDSettings.BACK_DEAD_END, LEDSettings.StateColors.LLDEAD));
        }
    }
