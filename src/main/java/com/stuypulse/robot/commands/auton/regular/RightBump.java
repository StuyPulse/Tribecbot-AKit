    ...
    public RightBump(PathPlannerPath... paths) {
        ...
        addCommands(
            ...
            new ParallelCommandGroup(
                ...
                new WaitCommand(0.5).andThen(Intake.getInstance().deploy()).andThen(new WaitCommand(1.0))
            ),
            ...
            // SOTM To Depot
            new WaitUntilCommand(() -> Superstructure.getInstance().atTolerance()),
            Handoff.getInstance().runHandoffForward().alongWith(Spindexer.getInstance().runSpindexerForward()),
            new ParallelCommandGroup(
                Drive.getInstance().followPathCommand(paths[2]),
                new WaitCommand(6.0).andThen(
                    Handoff.getInstance().stopHandoffCommand().alongWith(Spindexer.getInstance().stopSpindexer())
                )
            ),
            ...
            new ParallelCommandGroup(
                Handoff.getInstance().runHandoffForward().alongWith(Spindexer.getInstance().runSpindexerForward()),
                new WaitCommand(3.0).andThen(Intake.getInstance().autoDigest().repeatedly())
            )
