    ...
    public TwoCornerShallow(PathPlannerPath... paths) {
        ...
        addCommands(
            ...
            new ParallelCommandGroup(
                ...
                new WaitCommand(0.5)
                    .andThen(Intake.getInstance().autoDigest().until(() -> Superstructure.getInstance().isHopperEmpty()).withTimeout(4.0)),
                new WaitCommand(1.0).andThen(
                    new WaitUntilCommand(() -> Superstructure.getInstance().isHopperEmpty()).withTimeout(3.5))
            ),
            Superstructure.getInstance().autoInterpolation().alongWith(Intake.getInstance().setPivotStateCommand(PivotState.DEPLOY)),
